...
#'
#' `wt.mean` calculates the mean given a weighting of the values. \cr \cr
#' `wt.var` is the unbiased variance of the weighted mean calculation
#' using equations of GNU Scentific Library
#' (<http://www.gnu.org/software/gsl/manual/html_node/Weighted-Samples.htmland>.\cr\cr
#' `wt.sd` is the standard deviation of the weighted mean calculated as
#' the sqrt of `wt.var`. \cr \cr **Note:** NA data is automatically
#' ommitted from analysis.
#'
#'
#' @param x is a vector of numerical data.
#' @param wt is a vector of equal length to `x` representing the weights.)
