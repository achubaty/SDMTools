...
#'
#' `asc2dataframe` converts a list of Esri ascii grid formatted files to a
#' data.frame consisting of only locations with data.\cr\cr
#' `dataframe2asc` converts a data.frame or matrix with spatial data to
...
asc2dataframe <- function(filenames, varnames = NULL, gz = FALSE) {
  ...
  for (ii in 1:length(filenames)) { # cycle through each of the files
    tfile <- filenames[ii]
    varname <- varnames[ii] # define the file and variable name
    cat("working with", tfile, "...\n")
    if (!file.exists(tfile)) {
      warning(paste(tfile, "does not exist and was not used"))
      next
    } # check if the file exists, if not move to the next file
