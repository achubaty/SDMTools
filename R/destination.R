...
#'
#' `destination` estimates the destination latitude and longitude given a
#' starting latitude and longitude, a bearing and distance. \cr \cr For general
#' information on Vincenty's formula, see e.g.,
#' <http://en.wikipedia.org/wiki/Vincenty's_formulae>. It states: \cr
...
#' Earth.* \cr \cr **Note:** this method assumes a locations are lat & lon
#' given in WGS 84.
#'
...
#' <http://www.movable-type.co.uk/scripts/latlong-vincenty-direct.html>.\cr
...
#' destination(lats, lons,
#'   bearing = seq(0, 360, length.out = 9),
#'   distance = seq(0, 5000000, length.out = 11)
#' )
#'
#' ### multiple lat lons
#' lats <- seq(-90, 90, length.out = 9)
#' lons <- seq(-180, 180, length.out = 9)
#'
...
destination <- function(lat, lon, bearing, distance) {
  ...
  if (length(lat) == 1) {
    ...
    if (length(bearing) > 1) {
      if (length(bearing) != length(lat)) stop("number of bearing values is not the same length as lon & lat")
    } else {
      bearing <- rep(bearing, length(lat))
    }
    if (length(distance) > 1) {
      if (length(distance) != length(lat)) stop("number of distance values is not the same length as lon & lat")
    } else {
      distance <- rep(distance, length(lat))
    }
