...
#'
#' `getXYcoords` computes the geographical coordinates of the rows and
#' columns of pixels of a raster map of class `asc`. Code & helpfile were
#' modified from adehabitat package.
#'
#'
#' @param w an object of class `asc`.
...
getXYcoords <- function(w) {
  ...
  if (inherits(w, "RasterLayer")) w <- asc.from.raster(w)
  if (inherits(w, "SpatialGridDataFrame")) w <- asc.from.sp(w)
