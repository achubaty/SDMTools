...
#'
#' `read.asc` and `read.asc.gz` reads ESRI ArcInfo ASCII raster file
#' either uncompressed or compressed using gzip. \cr \cr `write.asc` and
#' `write.asc.gz` writes an asc object to a ESRI ArcInfo ASCII raster
...
#' export.asc.\cr\cr `write.asc2` and `write.asc2.gz` are even faster
#' implementations but have less error checking. \cr \cr `image.asc` and
#' `print.asc` are generic methods associated with plotting & summarizing
...
#' ascii grid file
#' @param col for maps of type `"numeric"`, the colors to be used (see
#' `help(par)`)
#' @param clfac for maps of type `"factor"`, a character vector giving the
#' names of colors for each level of the factor (see `help(colasc)`)
