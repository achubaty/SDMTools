...
#'
#' `distance` estimates the distance given a starting & ending latitude
#' and longitude. \cr \cr For general information on Vincenty's formula, see
#' e.g., <http://en.wikipedia.org/wiki/Vincenty's_formulae>. It states: \cr
...
#' <http://www.movable-type.co.uk/scripts/latlong-vincenty.html>.\cr \cr
#' Distances were validated against Geoscience Australia calculations
#' (<http://www.ga.gov.au/geodesy/datums/vincenty_inverse.jsp>).\cr \cr
...
distance <- function(lat1, lon1 = NULL, lat2 = NULL, lon2 = NULL, bearing = FALSE) {
  if (is.data.frame(lat1) | is.matrix(lat1)) { # if input is matrix or data.frame... break it out to individual vectors
    lat1 <- as.matrix(lat1)
    if (ncol(lat1) != 4) stop("incorrect lat/lon inputs... must be matrix with 4 columns or 4 vectors")
    lon2 <- lat1[, 4]
    lat2 <- lat1[, 3]
    lon1 <- lat1[, 2]
    lat1 <- lat1[, 1] # break out individual columns
