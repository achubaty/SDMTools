...
#'
#' `SigDiff` computes the significance of the pairwise differences
...
#' \cr `ImageDiff` is a wrapper to the image.asc command in adehabitat
#' package that uses the result from `SigDiff` to create an image mapping
#' the regions of significant differences (positive and negative). \cr \cr
#' **NOTE:** it is assumed the input data are of the same extent and
#' cellsize.
#'
#'
...
#' `SigDiff`; The matrix can be a raster of class 'asc' (adehabitat
...
#' @return `SigDiff` returns a vector or matrix of the same dimensions and
...
#' \cr \cr `ImageDiff` returns nothing but creates an image of the areas
...
#' legend("topleft",
#'   legend = c("-ve", "ns", "+ve"), title = "significance",
...
SigDiff <- function(x, y, pattern = TRUE) {
  ...
  if (inherits(x, "asc")) {
    ...
    x <- asc.from.raster(x)
    y <- asc.from.raster(y)
  } else if (inherits(x, "SpatialGridDataFrame")) {
    attrib <- x
    x <- asc.from.sp(x)
    y <- asc.from.sp(y)
  } else {
  ...
  if (pattern) {
    ...
    px <- x[pos]
  } # calculate the proportionate value relative to the sum of values across the raster
  if (pattern) {
    py <- y[pos] / sum(y[pos])
  } else {
    py <- y[pos]
  ...
  out <- x
  out[pos] <- t.sig # create the output ascii grid and write significance values to it

  # reset the attributes of the input
  if (inherits(attrib, "RasterLayer")) {
    attrib <- raster::setValues(attrib, as.vector(t(t(unclass(out))[dim(out)[2]:1, ])))
    return(attrib)
  } else if (inherits(attrib, "SpatialGridDataFrame")) {
