#' Confusion Matrix
#'
#' `confusion.matrix` calculates a confusion matrix. \cr \cr **Note:**
#' this method will exclude any missing data
#'
#'
#' @param obs a vector of observed values which must be 0 for absences and 1
#' for occurrences
#' @param pred a vector of the same length as `obs` representing the
...
#' @seealso [auc()], [Kappa()], [omission()],
#' [sensitivity()], [specificity()],
#' [prop.correct()], [accuracy()]
#' @examples
#'
#'
#' # create some data
...
confusion.matrix <- function(obs, pred, threshold = 0.5) {
  ...
  n <- length(obs)
  if (length(which(obs %in% c(0, 1, NA))) != n) stop("observed values must be 0 or 1") # ensure observed are values 0 or 1
