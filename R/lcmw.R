...
#' `mw` and the matrix `mat`, returning the minimum cost value. This
...
#' tasc <- as.asc(matrix(1:100, nr = 10, nc = 10))
#' print(tasc)
#'
...
#' tcost <- matrix(0, nr = 5, nc = 5)
#' print(tcost)
#' out <- lcmw(tasc, tcost, 2)
#' print(out[1:10, 1:10])
#'
...
#' # no cost matrix
#' tcost1 <- tcost
#' tcost1[is.finite(tcost1)] <- 1
#' print(tcost1)
#' out <- lcmw(tasc, tcost1, 2)
#' print(out[1:10, 1:10])
#'
#' # linear cost
#' tcost <- tcost / 2
...
lcmw <-
  function(mat, mw, mnc) {
    ...
    vals$cost <- mw[(mnc + 1) + cbind(vals$Y, vals$X)]
    vals <- na.omit(vals) # extract the cost of associated with the move
    nrow.vals <- nrow(vals)
