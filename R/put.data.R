...
#'
#' `put.data` replaces data in raster object of class 'asc' (this and
#' adehabitat package) at specified locations.\cr \cr **Note:** there is no
...
put.data <-
  function(pts, x) {
    ...
    pts <- as.matrix(pts)
    if (dim(pts)[2] != 3) stop("input pts must be 3 columns of data")
    xy <- getXYcoords(x)
