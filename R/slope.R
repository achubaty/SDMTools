...
#'
#' `slope` and `aspect` calculates the slope and aspect of raster
...
#' representing cellsize if `latlon`=FALSE or km if `latlon`=TRUE.
#' This can be changed to percentage (multiply by 100) or to degrees by ATAN (
#' `output` ) * 57.29578.\cr\cr Aspect returns the direction (0 to 360)
...
#' @return an object of the same class as `mat`.
...
slope <- function(mat, latlon = FALSE) {
  ...
  } else if (inherits(attrib, "RasterLayer")) {
    attrib <- mat
    mat <- asc.from.raster(mat)
  } else if (inherits(mat, "SpatialGridDataFrame")) {
    attrib <- mat
    mat <- asc.from.sp(mat)
  } else {
    attrib <- attributes(mat)
  ...
  if (!inherits(mat, "asc")) {
    stop('objects must be of class "asc"')
  } # check to ensure asc
  ...
  if (latlon) {
    ...
    width <- rowMeans(cbind(tt$top, tt$bottom)) / 1000
    height <- tt$side / 1000 # get the width and height of the cells in km (NOT m)
  } else {
    width <- height <- rep(attr(mat, "cellsize"), length(getXYcoords(mat)$y))
  ...
  if (inherits(attrib, "RasterLayer")) {
    attrib <- raster::setValues(attrib, as.vector(t(t(unclass(mat))[dim(mat)[2]:1, ])))
    return(attrib)
  } else if (inherits(mat, "SpatialGridDataFrame")) {
    attrib@data[1] <- as.vector(unclass(mat)[, dim(mat)[2]:1])
    return(attrib)
  } else {
...
aspect <-
  function(mat, latlon = FALSE) {
    ...
    if (inherits(mat, "asc")) {
      attrib <- attributes(mat)
    } else if (inherits(mat, "RasterLayer")) {
      attrib <- mat
      mat <- asc.from.raster(mat)
    } else if (inherits(mat, "SpatialGridDataFrame")) {
      attrib <- mat
