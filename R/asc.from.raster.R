...
#' \cr `raster.from.asc` and `sp.from.asc` does the reverse.\cr\cr
...
asc.from.raster <- function(x) {
  if (!inherits(x, "RasterLayer")) stop("x must be of class raster or RasterLayer")
...
asc.from.sp <- function(x) {
  ...
  if (!inherits(x, "SpatialGridDataFrame")) stop("x must be of class SpatialGridDataFrame")
